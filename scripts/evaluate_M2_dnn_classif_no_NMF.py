...
from python.models.mcem import MCEM_M2_noNMF
from python.models.models import DeepGenerativeModel, Classifier
from python.models.spp_estimation import timo_noise_estimation
...
def main():
    ...
    for i, (x_t, file_path) in enumerate(zip(test_data, file_paths)):
        ...
        Vb = timo_noise_estimation(x.cpu().detach().numpy().T, y_hat_soft.cpu().detach().numpy().T)
        Vb = Vb.T

        mcem = MCEM_M2_noNMF(X=x_tf, Vb=Vb, g=g_init, Z=Z_init, y=y_hat_hard,
        ...
        sf.write(output_path + '_' + os.path.dirname(classif_name) + '_noNMF_s_est.wav', s_hat, fs)
        sf.write(output_path + '_' + os.path.dirname(classif_name) + '_noNMF_n_est.wav', n_hat, fs)
